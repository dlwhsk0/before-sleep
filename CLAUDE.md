...
- 네모난 카드를 **2열로 엇갈리게** 쌓는다. 최신순.
  구현은 CSS `grid`가 아니라 **`columns-2`**다 — grid는 한 행의 높이를 서로 맞춰
  짧은 메모 옆에 빈 공간을 만드는데, columns는 위에서 아래로 흘려 담아 두 열이
  자연스럽게 엇갈린다. 카드에 `break-inside-avoid`를 줘서 열 경계에서 쪼개지지 않게 한다.
- **높이는 글자 길이에 그대로 따라가지 않는다.** 최소 `min-h-22`(5.5rem),
  최대는 `line-clamp-8`(여덟 줄에서 자름). 한 장이 화면을 다 먹으면 보드가 아니게 된다.
  전문은 길게 눌러 수정 화면에서 본다.
- **날짜는 항상 붙는다(연도까지).** 고를 일이 아니라 기록의 일부다.
  (`Memo.date`는 타입상 optional이지만 새로 만들 때는 늘 채운다 — 옛 데이터 때문에 남긴 것.)
- **보드에 지우기 버튼을 두지 않는다.** 실수로 눌러 날리므로, 길게 눌러 여는
  수정 화면 안에만 둔다.
